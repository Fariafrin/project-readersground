...
class ProductDAO:
    ...
    def upload(self, p):
        ...
        try:
            dbcursor.execute("INSERT INTO product VALUES('',%s,%s,%s,%s,%s,%s,%s)", [p.getName(),p.getWriter(),p.getGenre(),p.getRate(),p.getReview(),p.getUpldate(),p.getImgpath()])
        except:
    ...
    def update(self, p):
        dbcursor=self.getDBCursor()
        try:
            dbcursor.execute("UPDATE product SET name=%s, writer=%s,genre=%s, rate=%s, review=%s, upldate=%s WHERE id=%s", [p.getName(),p.getWriter(),p.getReview(),p.getUpldate(),p.getId()])
        except:
            raise Exception('data insertion error')
        finally:
            dbcursor.close()
